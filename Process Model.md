 ...
 * Start Process: The start process is load-balanced and may run on a different execution engine.
...
## 5. What are the ways to start a Process Model?
 * Using Actions
 * Using Sites
...
## 6. What are the recommended best-practice limits for the number of nodes and process variables in process model?
30 Nodes and 50 Variables

## 7. What is MNI and where do you use them?
...
## 10. What are gates and its type? Explain with real time use case?
 ...
 * Using a smart service send document to external system 
...
## 19. Third Part want to trigger an Stores Procedure? how do you implement them?
Create WebAPI of start process type and implement using Execute Stored Procedure smart service

## 20. How to schedule a PM at specifi time?
In start node by scheduling the time and interval of run 
...
## 23. Does activity chaining decrease the Performance, if yes how do you overcome?
Yes, excessive activity chaining can cause CPU spikes and performance impact as it creates pressure for faster execution; use activity chaining only for priority or required steps

## 24. Is an archived or deleted process available in the Process report?
No. Deleted process instances are permanently removed and cannot be restored, archived process instances can be unarchived and made available in reports by using the Self-Service Process Unarchive (SSPU) plugin, which is used to unarchive archived process instances.

## 25. There is a 1000 Instances on the Production and you want to edit the variable value, how can we achieve it instad of manual approach?
...
## 26. Usage of Lane in PM?
For Easy readability to separate between User input task and system acted task

## 27. How will you deploy a scheduler PM?
...
## 28. How do you handle retry machanism in PM for integration?
Setting Up retry after failed attempt after (mostly when status code is failed with 5XX) few minutes and also write the log for the integration when ever hits for analysis.

## 29. What is the difference between End Node vs terminate Node?
...
## 30. What is Activity Chaining and What are its limitations?
...
## 31. What is the difference between User Input Task and Process Start Form? When should one use Process Start Form?
...
## 32. What is the purpose of lane assignments?
...
## 33. Why should we build short-lived processes?
...
## 34. What is the purpose of alerts in Process model?
...
## 35. Why do we need the data management? When should one select the delete option in data management?
...
## 36. What is the difference between end and terminate node? When to use End node?
...
## 37. What are the different types of process report?
 * Process models
 * Process instances
 * Active tasks

## 38. What is the function used to get the results from process report?
a!queryProcessAnalytics()

## 39. What is the purpose of activity chaining? What is the maximum limit?
Activity chaining is used between more than one task performed by the same user so that the user can act upon the tasks without any delay or break. The maximum limit is default 50 and max 100 unattended nodes between two attended nodes.

## 40. When a process with an active task is paused, will the task be available to the user to perform?
No. An error will be shown to the user when the task is opened.

## 41. Will timer node execute when the instance is paused?
Yes

## 42. What is a MNI and its best practices?
...
## 43. The triggers that can be added to a start event are?
 * Receive Message
 * Timer

## 44. What is the difference between Task Assignee and Task Owner?
